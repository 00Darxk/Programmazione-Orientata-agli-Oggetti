...
In questo modo è possibile istanziare un oggetto di tipo scelto a tempo dinamico. Poicé restituisce un oggetto classe, di tipo generico sconosciuto, è 
necessario effettuare un downcast al tipo voluto, a tempo di compilaziove viene sollevato un'avvertenza ``Unchecked Cast Warning'', poiché il 
compilatore non è in grado di definire chiaramente il funzionamento del codice. 

Questi avvertimenti sono dovuti alla lasca tipizzazione, e alla tardiva impelmentazione dei generics nel linguaggio. Per cui il compilatore deve mantenere 
...
Questo può essere necessario quando si utilizzano collezioni che mischiano gerachia di tipi diverse tra di loro, quindi all'interno del metodo \verb|equals()| 
...
\end{minted}
Per cui è opportuno includere anche la classe nella funzione di hash, avente una suo funzione di hash di defulat che è possibile utilizzare:
\begin{minted}{java}
    @Override
    public int hashCode(){
...
stessa gerarchia. 
La definizione di un criterio di equivalenza deve quindi tenere conto dell'intera gerarchia dei tipi. Questo però rappresnta un forte ed 
...
La definizione di un opportuno criterio di qeuivalenza rappresenta un importante problema di modellazione. 
...
sottotipo di \verb|Throwable|. Queste eccezioni possono decorate da altri messaggi e riferimenti ad altre eccezioni che l'hanno causato. %% TODO metodi Throwable (costruttore) per stack tracing
...
Per ogni distinta condizione anomala che si potrebbe incontrare si definisce una nuvoa eccezione o si riutilizza un'eccezione già presente. 
...
%% TODO img gerarchia InputStream e OutputStream
